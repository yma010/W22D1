...
function pow(base, exponent) {
    ...
    if (exponent < 0) {
        ...
        return base * pow(base, Math.abs(exponent - 1));
    }
...
function fileFinder(directories, targetFile) {
    for (let targetDir in directories) {
        if (targetDir === targetFile || fileFinder(directories[targetDir], targetFile) === true) {
            return true;
        }
    }
...
function pathFinder(directories, targetFile) {
    for (let targetDir in directories) {
        ...
        let path = pathFinder(directories[targetDir], targetFile);
        if (path !== null) {
